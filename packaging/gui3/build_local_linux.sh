...
ROOT="${BUNDLE_DIR}/tile_compile_gui3-linux-${TAG}"
...
assemble_bundle() {
  ...
  if [[ -f "${PROJECT_ROOT}/agent_service/package.json" ]]; then
    ...
    if ! command -v npm >/dev/null 2>&1; then
      echo "[gui3-package] npm is required to package agent_service." >&2
      exit 1
    fi
    (
  ...
  (
    cd "${BUNDLE_DIR}"
    zip -r "${ARTIFACTS_DIR}/tile_compile_gui3-linux-${TAG}.zip" "tile_compile_gui3-linux-${TAG}"
  )
}
...
smoke_test() {
  ...
  rm -rf "${HOME}"
  mkdir -p "${HOME}"
  bash "${root}/start_gui3.sh" >/tmp/out_gui3_smoke_linux.txt 2>&1 &
  local start_pid=$!
...
main() {
  ...
  echo "[gui3-package] Created ${ARTIFACTS_DIR}/tile_compile_gui3-linux-${TAG}.zip"
}
