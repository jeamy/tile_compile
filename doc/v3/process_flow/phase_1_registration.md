...
> **C++ Implementierung:** `runner_main.cpp` Zeilen 902–1158
> **Phase-Enum:** `Phase::REGISTRATION`

## Übersicht

Die Registrierung richtet alle Frames geometrisch auf einen Referenz-Frame aus. Die C++ Implementierung verwendet eine **5-stufige Registrierungskaskade** mit robusten Fallbacks. Anschließend werden alle Frames **vollständig pre-warped** auf Bildauflösung, bevor Tiles extrahiert werden.
...
│     ├─ Star Pair Matching     → OK? → accept         │
│     ├─ AKAZE Feature Match    → OK? → accept         │
│     ├─ Phase-Corr + ECC      → OK? → accept         │
...
## 3. Registrierungskaskade (5 Stufen)
...
```cpp
RegistrationResult fr = registration::triangle_star_matching(
    mov_reg, ref_reg, allow_rotation, star_topk, star_min_inliers, star_inlier_tol_px);
```

- **Rotationsinvariant**: Verwendet Dreiecke aus Sternpositionen
- **Konfiguration:** `star_topk` (Top-K Sterne), `star_min_inliers`, `star_inlier_tol_px`
- **Ideal für:** Alt/Az-Montierungen mit Feldrotation
- **Keine Rotationslimits** (entfernt wegen Alt/Az nahe Pol mit >20° Rotation)

### Stufe 2: Star Pair Matching (Schnell)

```cpp
RegistrationResult fr = registration::star_registration_similarity(
    mov_reg, ref_reg, allow_rotation, star_topk, star_min_inliers,
    star_inlier_tol_px, star_dist_bin_px);
```

- **Optimiert für kleine Shifts**: Sternpaare mit ähnlicher Distanz
- `star_dist_bin_px` steuert die Bin-Breite für Distanz-Histogramme

### Stufe 3: AKAZE Feature Matching

```cpp
RegistrationResult fr = registration::feature_registration_similarity(
    mov_reg, ref_reg, allow_rotation);
```

- **Feature-basiert**: AKAZE Keypoints + Descriptor Matching
- **Rotationsinvariant**: Keine Rotationslimits
- **Fallback** wenn zu wenige Sterne (z.B. dichte Nebel)

### Stufe 4: Hybrid Phase-Correlation + ECC

```cpp
RegistrationResult fr = registration::hybrid_phase_ecc(
    mov_reg, ref_reg, allow_rotation);
```

- **Phase-Correlation** für grobe Schätzung, dann **ECC-Refinement**
- Akzeptiert nur wenn `correlation >= 0.15`
- **Letzter Versuch** vor Identity-Fallback

### Stufe 5: Identity-Fallback
...
| `registration.star_topk` | Top-K Sterne für Matching | 50 |
| `registration.star_min_inliers` | Mindest-Inlier für Akzeptanz | 5 |
| `registration.star_inlier_tol_px` | Inlier-Toleranz in Pixel | 3.0 |
| `registration.star_dist_bin_px` | Distanz-Bin für Star Pairs | 5.0 |
...
| Alle 4 Methoden fehlgeschlagen | Identity-Warp, CC=0 (Frame bleibt!) |
