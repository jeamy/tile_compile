...
Final result per channel (optimized, statistically consistent):

Let `n_k` be the number of original frames assigned to cluster `k` (so `sum_k n_k = N`).

`R_c = sum_k (n_k * S_{k,c}) / sum_k n_k`

Optional refinement (if desired): introduce a cluster-level quality weight `Q_k` (e.g., mean of `G_{f,c}` in cluster `k`):

`R_c = sum_k (n_k * Q_k * S_{k,c}) / sum_k (n_k * Q_k)`

Optional before that: pixel-wise sigma clipping over `S_{k,c}` with fallback to unchanged mean in case of excessive rejection.
...
11. final stacking proportional to cluster size (cluster-weighted mean, not uniform per-cluster mean)
