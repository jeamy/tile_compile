# HTML Frontend + Crow/C++ Backend (Desktop)
...
- Crow/C++ Backend als API- und Prozess-Adapter
...
- `Frontend (HTML/CSS/JS)`:
  ...
  - kommuniziert nur ueber HTTP/WebSocket mit dem Crow-Backend
- `Crow/C++ Backend`:
...
4. Das Crow-Backend startet CLI/Runner-Prozesse, streamt Status zur UI.
...
Fuer HTML+Crow werden Runner/CLI und das Web-Backend benoetigt:
...
Pflicht (Stats/Report):

- integrierte C++ Report-Erzeugung
...
cd /media/data/programming/tile_compile/web_backend_cpp
...
brew install cmake ninja pkg-config eigen opencv cfitsio yaml-cpp nlohmann-json openssl curl
```
...
pacman -S --needed mingw-w64-x86_64-yaml-cpp mingw-w64-x86_64-nlohmann-json mingw-w64-x86_64-openssl mingw-w64-x86_64-curl
```

Build (MSYS2 MinGW64 Shell, Qt-freies Zielprofil):
...
## 5) C++-Runtime und Packaging

Empfehlung:

- Runner/CLI und Web-Backend zusammen bauen
- keine Python-Runtime im produktiven GUI2-Pfad
- Frontend als statische Assets gemeinsam mit dem Crow-Backend ausliefern

Empfohlenes Layout:

- `web_backend_cpp/build/tile_compile_web_backend`
- `tile_compile_cpp/build/tile_compile_runner`
- `tile_compile_cpp/build/tile_compile_cli`
- `web_frontend/`
...
| `POST` | `/api/runs/{run_id}/stats` | Report erstellen | `tile_compile_cli generate-report <run_dir>` |
...
- klassischer CMake-Build fuer `web_backend_cpp`
- Frontend-Dateien werden als statische Assets bereitgestellt
...
- `dist/web/` fuer Crow-Backend + Frontend-Assets
- gemeinsamer Launcher pro OS:
  - startet das Crow/C++-Backend
  - oeffnet Browser-URL
...
2. `tile_compile_runner`, `tile_compile_cli` und `tile_compile_web_backend` gebaut und startbar.
3. Backend kann Runner/CLI-Pfade lesen.
4. Frontend laedt unter `http://127.0.0.1:8080/`.
5. Scan, Validate, Run, Resume, History, Generate Stats erfolgreich getestet.
...
- Prioritaet fuer Migration:
  ...
  2. Crow-Backend stabilisieren (subprocess, timeouts, error mapping).
  3. End-to-End Tests fuer Wizard + Resume + Queue.
  4. Danach Packaging verfeinern.

Damit bleibt die C++ Engine unveraendert nutzbar, waehrend die GUI auf ein wartbares HTML/Crow-Modell umgestellt wird.
