...
**Implementation Status:** Enforced via `PolicyValidator.validate_linearity()`
...
```yaml
registration:
  max_residual_px: 0.5  # Maximum allowed registration residual
  residual_action: warn  # or: abort
```

**New Validation:**
- Compute residual statistics from star matches
- Warn if median residual > threshold
- Abort if max residual > 2× threshold
...
```yaml
data:
  frames_min: 50           # Hard minimum
  frames_optimal: 800      # Full methodology
  reduced_mode_threshold: 200
```

**Reduced Mode Behavior:**
- Skip state clustering
- Use global weights only (no synthetic frames)
- Output warning in validation report
...
assumptions:
  ...
  tracking_elongation_max: 0.3
  focus_drift_action: warn  # or: abort, ignore
```
