...
- [~] **2D-Kachelung + Parität + Host-Budget (Schritt 4) + Memo-Entfernung
  ...
  - [x] **Schritt 3a-2 — Q-Rechteckansichten (§30.81, `8db3f70a`+`1c365344`):**
        `read_rect(y0,y1,x0,x1)` + `FrameQualityMaps.y_origin/x_origin` +
        `FrameQualityRectProvider`. `reduce_window` holt Q für **exakt** die
        Record-Quell-Bbox (keine Schätzung/Marge → Lokale-Warp-Bbox-Problem
        entfällt). Store-Provider `read_rect`. Veto/NaN + Randklammer
        byte-identisch (`[cuda-parity]` echtes Gerät + `read_rect`-Unit-Test).
  - [x] **Schritt 3a-2b — seek-Read (§30.81, `cf2b4e21`):** `read_rect`
        dekodiert nur die überdeckenden Speicherzellen (je Zell-Zeile ein
        seek+read Wert- + Veto-Block). `read_bin`→`read_bin_header`+
