# Tile-basierte Qualitätsrekonstruktion für DSO - Methodik v3.3.8
...
3. präzise Semantik für
   ...
   - runtime-konfigurierbare Betriebsmodi,
   - und den aktiven Schätzer für lokale Scores und Tile-Rekonstruktion
...
"Strikt linear" bedeutet in v3.3.6:
...
Frühere Methodikfassungen verwendeten die Bezeichnungen `strict` und `practical`. In v3.3.8 werden sie als **Implementierungsvarianten** behandelt, nicht als aktive Betriebsprofil-Schlüssel in `tile_compile.yaml`.
...
Optional/feature-gated: lokale Denoiser, alternative Post-Stack-Clipping-Strategien, WCS/PCC.
...
### 5.5.4 Nachbarschaftsbewusste Metrik-Normalisierung (verbindlich in v3.3.8)
...
### 5.5.5 Räumliche Regularisierung lokaler Scores (verbindlich in v3.3.8)

Zuerst wird der unregularisierte lokale Score berechnet:

`Q_{f,t,c}^{raw} = clip(Q_{f,t,c}^{star|struct}, q_min, q_max)`

mit dem lokalen Default-Clip-Bereich `[q_min, q_max] = [-3, +3]`.
...
`V_{t,c}(p) = { f | I_{f,c}(p) ist endlich und > 0 }`.
...
Für ein rekonstruiertes Tile `R_{t,c}` sei

`V_t^+ = { p | R_{t,c}(p) endlich und > 0 innerhalb des gültigen Rekonstruktionssupports }`.
...
4. wenn `m_t >= eps_median`: `Y_t(p) = X_t(p) / m_t`, sonst `Y_t(p) = X_t(p)`
...
#### 5.7.1b Photometrische Erhaltung nach OLA (verbindlich in v3.3.8)
...
- Pro Tile (bereits berechnet): `bg_{t,c}`, `m_t`
- Globale Restaurationsfaktoren (robust):
  - `m_global = median_t(m_t)`
  - `bg_global,c = median_t(bg_{t,c})`

Nachdem OLA `I_rec,c` erzeugt hat, restauriere:

`I_final,c = I_rec,c * m_global + bg_global,c`

Für Monodaten reduziert sich das auf den Ein-Kanal-Fall. Dadurch bleibt der Core affin in den Pixelwerten, während systematischer tileweiser photometrischer Drift vermieden wird.
...
Nach OLA darf die aktive Runtime die globale affine Restaurierung aus §5.7.1b anwenden, bevor spätere Ausgabeskalierung oder nachgelagerte Post-Processing-Schritte folgen.
...
- **Self-supervised Learning:** Konsistenz über zufällige Frame-Subsets, Noise2Self/Noise2Void-artige Ziele für Masken/Denoise-Proxys (Hinweis: Denoising darf innerhalb des verpflichtenden Rekonstruktionskerns weiterhin nur Masken/Gewichte, nicht Pixel, ausgeben).
