...
| `reconstruction.clipping.bimodal_veto` | boolean | `false` | Läuft sowohl CPU- als auch CUDA-beschleunigt. Prüft nach Konvergenz des Median/MAD-Clip-Durchgangs die akzeptierte Menge auf eine kohärente zweite Population, die die gewöhnliche Grenze allein nicht von der Mehrheit getrennt hat (z. B. Frames mit einem kleinen, realen Registrierungs-/Nachführ-Offset an genau diesem Pixel) -- wahrscheinlicher bei weiten `clip_sigma_low`/`clip_sigma_high`, die bewusst gelockert sind, um echte Signalstreuung zu erhalten, und dadurch auch eine kohärente Minderheit leichter zulassen. Übersteigt die größte Lücke zwischen aufeinanderfolgenden akzeptierten Werten `bimodal_veto_gap_sigma` mal die MAD des Durchgangs und trennt dabei eine Minderheitsseite mit mindestens 2 Kandidaten und unter der Hälfte des akzeptierten Gewichts ab, wird diese Minderheit verworfen. Aktualisiert bei Auslösung zusätzlich die eingefrorenen Pilot-Grenzen des Voll-Frame-Schätzers, damit die verworfene Population nicht über spätere Nicht-Pilot-Frames wieder zugelassen wird. |
| `reconstruction.clipping.bimodal_veto_gap_sigma` | number | `2.5` | `> 0`. Vielfaches der MAD des Clip-Durchgangs, das die größte Lücke zwischen akzeptierten Werten überschreiten muss, damit `bimodal_veto` die Trennung als kohärente zweite Population statt gewöhnlicher Streuung wertet. Kleinere Werte lösen leichter aus. |
...
| `hypermetric_stretch.color_cast_correction.brightness_bins` | integer | `8` | Helligkeitsklassen der Objektpixel, [1, 32]; die Stärke wird je Klasse gewählt und über die Pixelhelligkeit interpoliert (1 = eine globale Stärke). |
| `hypermetric_stretch.color_cast_correction.neutralize_sky` | boolean | `false` | Verschiebt zusätzlich G, sodass der Himmel neutral ist: `G += (Himmel R + Himmel B)/2 - Himmel G` (übersprungen ab 25 % Offset). |
...
| `hypermetric_stretch.large_scale_contrast.enabled` | boolean | `false` | Hebt nur die großräumige Himmelsstruktur (Staubbänder, Reflexionsnebel) im gestreckten Bild an; kein zusätzliches Rauschen, keine Änderung der Sterne. Läuft nach der Farbstichkorrektur. |
| `hypermetric_stretch.large_scale_contrast.amount` | number | `1.0` | Faktor auf die großräumige Abweichung vom Himmelspegel, [0, 6]; 1 verdoppelt, 2 verdreifacht die sichtbare Struktur (Zuwachs bei sigma_px 48 etwa 60 % davon). |
| `hypermetric_stretch.large_scale_contrast.sigma_px` | number | `48.0` | Gauß-Sigma der „großen“ Komponente in Ausgabepixeln, (0, 512]; Strukturen darunter bleiben unberührt. |
| `hypermetric_stretch.large_scale_contrast.chroma_amount` | number | `0.0` | Gleicher Faktor für die großräumigen Farbunterschiede R−G und B−G, [0, 6]; G bleibt unverändert; 0 = Farbe unverändert. |
| `hypermetric_stretch.large_scale_contrast.remove_vignette` | boolean | `true` | Nimmt eine radialsymmetrische Komponente (Vignette, radiales Himmelsleuchten) von der Verstärkung aus; bei zentrierten, radialsymmetrischen Objekten ausschalten. |
...
- `reconstruction.clipping.bimodal_veto`, `bimodal_veto_gap_sigma`: verwirft eine kohärente Minderheitspopulation (z. B. eine von Registrierungsdrift betroffene Frame-Untermenge), die die gewöhnliche weite Clip-Grenze allein zulässt; siehe §14-Tabelle oben.
