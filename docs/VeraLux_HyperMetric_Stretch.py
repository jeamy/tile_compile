...
# VERSION HISTORY
# ------------------------------------------------------------------------------
# 1.5.3: Robustness & Correctness Patch.
#        • Fix: Thread-Safety. Added threading.Lock (_diag_lock) to protect all
#          reads and writes of _last_linear_expansion_diag, preventing race
#          conditions between ProcessingThread and the Main (GUI) Thread.
#        • Fix: normalize_input now uses posinf=1.0 instead of posinf=None,
#          ensuring +Inf values are clamped rather than propagated downstream.
#        • Fix: apply_linear_expansion now guards the np.unravel_index call
#          behind a data.ndim == 2 check, preventing a crash when called with
#          a 1D subsampled array (e.g., from the preview proxy pipeline).
#        • Fix: adaptive_output_scaling now works on an internal copy of the
#          input array, preventing unintended in-place mutation of the caller's
#          data in future code paths.
#        • Fix: Solver loop restructured to exit immediately for Scientific Mode
#          before the RTU-only star-pressure damping loop, removing dead code
#          that was silently executing (but having no effect) in Scientific Mode.
#        • Fix: zoom_1_1 now sets an explicit scale(1.0, 1.0) transform instead
#          of resetTransform(), ensuring true 1:1 pixel mapping at any prior zoom.
#        • Fix: linear_expansion is no longer written to QSettings on close,
#          making the "always reset to 0.0" behavior consistent and intentional.
#        • Cleanup: calculate_anchor merged the two identical RGB branches
#          ((3,H,W) and (3,N)) into a single condition with 'or'.
...
class VeraLuxCore:
    ...
    # Protected by a lock to prevent race conditions between ProcessingThread and Main Thread.
    ...
    def calculate_anchor(data_norm):
        ...
        if (data_norm.ndim == 3 and data_norm.shape[0] == 3) or \
           (data_norm.ndim == 2 and data_norm.shape[0] == 3):
            floors = []
    ...
    def apply_linear_expansion(data: np.ndarray, factor: float) -> np.ndarray:
        ...
        Supports 2D (H,W) and 1D (N,) arrays. 1D arrays skip the spatial NN check.
        """
        if factor <= 0.001:
            with VeraLuxCore._diag_lock:
        ...
        if abs_max > 0.001 and data.ndim == 2:
            # Nearest Neighbor Check — only valid for 2D spatial arrays
            ...
            # If it's a real star (bright neighbours), use Absolute Max -> ZERO CLIPPING
            if neighbors.size > 0:
                max_neighbor = np.max(neighbors)
                if max_neighbor >= (abs_max * 0.20):
                    use_absolute_max = True

        elif abs_max > 0.001 and data.ndim == 1:
            # 1D (subsampled) data: no spatial context, use percentile fallback only.
            # use_absolute_max stays False.
            pass
...
class VeraLuxPreviewWindow(QWidget):
    ...
    def set_image(self, qimg, img_data_for_hist, source_saturated=False, mode="ready_to_use", luma_weights=None):
        ...
        with VeraLuxCore._diag_lock:
            diag = dict(VeraLuxCore._last_linear_expansion_diag)
    ...
    def zoom_1_1(self):
        # Set the view transform to exactly 1:1 pixel mapping (scale = 1.0).
        # resetTransform() would only be correct if the view was never scaled;
        # here we explicitly set scale to 1.0 regardless of current zoom level.
        self.view.setTransform(self.view.transform().fromScale(1.0, 1.0))
...
class AutoSolverThread(QThread):
    ...
    def run(self):
        try:
            ...
            # Scientific Mode: single-pass solve, no adaptive scaling simulation needed.
            ...
            # Ready-to-Use Mode: iterative "Floating Sky Check" with star-pressure awareness.
            # Max 15 iterations
            for _ in range(15):
                best_log_d = VeraLuxCore.solve_log_d(valid, target_temp, self.b_val)

                # --- STAR-AWARE ADAPTATION ---
                # If stellar pressure is high, reduce aggressiveness to avoid
                # star-driven over-compression of the background.
...
class VeraLuxInterface:
    def __init__(self, siril_app, qt_app):
        ...
        # Always start at 0.00 for Scientific safety (never persisted).
        self.spin_exp.setValue(0.0)
        self.slide_exp.setValue(0)
    ...
    def handle_close_event(self, event):
        ...
        # Note: linear_expansion is intentionally NOT persisted (always resets to 0.0 for safety).
