...
//                       passed to compute_source_quality_map()
...
//                      geometry that compute_source_quality_map() operates on
...
// compute_source_quality_map (log-accumulation) and by any per_scale_hook
...
inline SourceQualityMapResult compute_source_quality_map(
    const Matrix2Df &frame, const std::vector<uint8_t> &canvas_mask,
    int canvas_mask_width, int canvas_mask_height,
    const config::ReconstructionQualityPyramidConfig &cfg,
    core::AccelerationBackend backend = core::AccelerationBackend::cpu,
    cv::cuda::Stream *stream = nullptr) {
  return compute_source_quality_map(frame, canvas_mask, {}, canvas_mask_width,
