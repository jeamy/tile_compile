...
Config Config::from_yaml(const YAML::Node &node) {
  ...
  if (node["reconstruction"]) {
    ...
    if (yaml_has_value(r["drizzle"])) {
      ...
      if (yaml_has_value(d["full_frame_estimator"]))
        rc.drizzle.full_frame_estimator = d["full_frame_estimator"].as<bool>();
    ...
    if (yaml_has_value(r["clipping"])) {
      ...
      if (yaml_has_value(c["bimodal_veto"]))
        rc.clipping.bimodal_veto = c["bimodal_veto"].as<bool>();
      if (yaml_has_value(c["bimodal_veto_gap_sigma"]))
        rc.clipping.bimodal_veto_gap_sigma =
            c["bimodal_veto_gap_sigma"].as<float>();
    }
...
YAML::Node Config::to_yaml() const {
  ...
  // Single-method reconstruction contract (plan 6.1).
  {
    ...
    r["drizzle"]["full_frame_estimator"] = rc.drizzle.full_frame_estimator;
    ...
    r["clipping"]["shared_frame_rejection_consensus"] =
        rc.clipping.shared_frame_rejection_consensus;
    r["clipping"]["bimodal_veto"] = rc.clipping.bimodal_veto;
    r["clipping"]["bimodal_veto_gap_sigma"] =
        rc.clipping.bimodal_veto_gap_sigma;
...
void ReconstructionConfig::validate() const {
  ...
  req(clipping.bimodal_veto_gap_sigma > 0.0f,
      "clipping.bimodal_veto_gap_sigma must be > 0");
