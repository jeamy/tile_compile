...
#include "tile_compile/metrics/robust_stats.hpp"
...
std::vector<StackValidationStarSample> detect_validation_stars(
...
void measure_star_tail_metrics_at_samples(
    const Matrix2Df &image,
    const std::vector<StackValidationStarSample> &stars,
    const std::vector<uint8_t> &validation_mask, StackValidationMetrics &out) {
...
StackValidationMetrics measure_stack_validation_metrics_at_samples(
    const Matrix2Df &image,
    const std::vector<StackValidationStarSample> &stars,
    const std::vector<uint8_t> &validation_mask) {
  StackValidationMetrics out;
...
StackValidationReference prepare_stack_validation_reference(
    const Matrix2Df &image, const std::vector<uint8_t> &validation_mask) {
  StackValidationReference reference;
  ...
  reference.metrics = measure_stack_validation_metrics_at_samples(
...
StackValidationComparison compare_stack_to_reference(
    ...
    const std::vector<uint8_t> &validation_mask) {
  StackValidationComparison out;
  if (candidate.cols() != control.width || candidate.rows() != control.height) {
    return out;
  }
  out.candidate = measure_stack_validation_metrics_at_samples(
      candidate, control.stars, validation_mask);
  out.control = control.metrics;
  out.seam_score_regression = regression(out.candidate.seam_score,
                                         out.control.seam_score);
  out.fwhm_regression = regression(out.candidate.fwhm, out.control.fwhm);
  out.background_rms_regression = regression(out.candidate.background_rms,
                                             out.control.background_rms);
  out.tail11_abs_regression = regression(out.candidate.tail11_abs_median,
                                         out.control.tail11_abs_median);
  out.elongation_regression = regression(out.candidate.elongation_median,
  ...
  out.fwhm_applicable = out.candidate.fwhm > 0.0f && out.control.fwhm > 0.0f;
  const float seam_eps = metrics::eps_scale(
      {out.candidate.seam_score, out.control.seam_score});
  out.seam_applicable =
      std::isfinite(out.candidate.seam_score) &&
  ...
  const float background_eps = metrics::eps_scale(
      {out.candidate.background_rms, out.control.background_rms});
  out.background_rms_applicable =
      std::isfinite(out.candidate.background_rms) &&
  ...
  out.tail_applicable = out.candidate.star_count >= min_stars_for_tail &&
