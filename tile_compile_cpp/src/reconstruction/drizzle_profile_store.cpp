...
DrizzleStoreIdentity make_drizzle_store_identity(
  ...
  config::ReconstructionDrizzleConfig budget_cfg = cfg;
  const auto dims = plan_drizzle_memory_autogrow(
      plan, budget_cfg, 1, 0, true, [](const std::string &msg) {
        std::cout << "[DRIZZLE_STORE][warn] " << msg << std::endl;
      });
...
DrizzleStoreResult persist_forward_drizzle_uniform(
  ...
  if (cfg.internal_scale == 2 && cfg.output_scale == 1)
    throw std::invalid_argument(
        "DRIZZLE_STORE_UNIFORM_ONLY_2_1_UNSUPPORTED");
