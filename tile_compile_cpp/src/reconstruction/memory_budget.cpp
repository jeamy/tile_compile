    ...
    int    requested_workers)
{
    ...
    // OLA output buffers: accum + weight_sum, both float, for all tiles.
    const size_t tile_pixel_bytes =
        ...
        sizeof(float);
    const size_t tile_batch_bytes = static_cast<size_t>(num_tiles) * tile_pixel_bytes * 2;

    if (tile_batch_bytes >= memory_budget_bytes) {
        throw ReconstructionError(
            ReconstructionError::Code::memory_budget_too_small_for_tiles,
            "OLA output buffers (" + std::to_string(tile_batch_bytes / (1024 * 1024)) +
    ...
    const size_t available_for_frames =
        static_cast<size_t>(memory_budget_bytes * 0.8) - tile_batch_bytes;
    ...
    int effective_workers = requested_workers;
    ...
    plan.allocated_tile_batch_bytes  = tile_batch_bytes;
