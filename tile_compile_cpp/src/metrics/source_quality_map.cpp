...
#define TILE_COMPILE_SQMAP_CUDA_FILTERS 1
#else
#define TILE_COMPILE_SQMAP_CUDA_FILTERS 0
#endif

#if __has_include(<opencv2/core/ocl.hpp>)
#include <opencv2/core/ocl.hpp>
#define TILE_COMPILE_SQMAP_OPENCL 1
#else
#define TILE_COMPILE_SQMAP_OPENCL 0
#endif
...
// -ffast-math (which would invalidate quality-map NaN/canvas semantics).
...
Matrix2Df source_masked_frame(const Matrix2Df &frame,
  ...
  if (use_mask && !mask_shape_valid) {
    std::cerr << "[SQMAP] Warning: canvas mask shape mismatch — mask "
              << mask_w << "x" << mask_h << " (" << canvas_mask.size()
              << ") vs frame " << frame.cols() << "x" << frame.rows()
              << " (" << frame.size() << ")\n";
  }
  if (use_frame_mask && !frame_mask_shape_valid) {
    std::cerr << "[SQMAP] Warning: frame mask size mismatch — mask "
...
#pragma omp simd
  ...
  try {
    if (backend == core::AccelerationBackend::opencv_cuda) {
#if TILE_COMPILE_SQMAP_CUDA_FILTERS
...
Matrix2Df phi_artifact(const Matrix2Df &img, const Matrix2Df &blur, int r,
                       float artifact_sigma, float max_artifact_fraction) {
  // Separable O(W*H) fast path (restored from pre-v0.2):
...
#pragma omp simd
  for (std::ptrdiff_t i = 0; i < n; ++i) {
    ...
    outlier_data[i] = valid ? ((std::abs(hp_data[i]) > artifact_sigma * tau)
                                   ? 1.0f
                                   : 0.0f)
                              : nan_value();
  }
  ...
  const float safe_frac_max = std::max(max_artifact_fraction, global_eps_scale);
#pragma omp simd
  for (std::ptrdiff_t i = 0; i < n; ++i) {
...
Matrix2Df robust_zscore(const Matrix2Df &m) {
  ...
  const float eps = std::max(eps_floor, metrics::robust_eps_rel * std::max(std::abs(med), mad));
...
Matrix2Df compute_psi(const Matrix2Df &sharp, const Matrix2Df &snr,
                      ...
                      const config::ReconstructionQualityPyramidConfig &cfg) {
  ...
  for (int y = 0; y < out.rows(); ++y) {
    for (int x = 0; x < out.cols(); ++x) {
      ...
      const float score = cfg.score_scale *
          (cfg.sharpness_weight * z_sharp(y, x) + cfg.snr_weight * z_snr(y, x));
...
// compute_source_quality_map can run it ONCE per scale and hand the result to
...
Matrix2Df compute_source_local_variance(const Matrix2Df &image, int radius) {
  return local_variance_linear(image, radius);
}

SourceQualityMapResult compute_source_quality_map(
  ...
  for (int s = 0; s < cfg.scales; ++s) {
    ...
    if (backend == core::AccelerationBackend::cpu) {
      sharp = compute_source_local_variance(laplacian, radius);
    ...
    const Matrix2Df artifact = phi_artifact(
        img_s, local_img.mean, radius, cfg.artifact_sigma,
        cfg.max_artifact_fraction);
