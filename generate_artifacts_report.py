...
def _write_css(path: Path) -> None:
    css = """
:root { --bg:#0b0f14; --fg:#e6edf3; --muted:#9aa7b2; --card:#111823; --border:#243044; --accent:#58a6ff; --warn:#ffb86c; --bad:#ff5555; --ok:#3fb950; }
...
def _write_html(path: Path, title: str, meta_lines: list[str], sections: list[tuple[str, list[str]]]) -> None:
    head = f"""<!doctype html>
<html lang=\"en\">
<head>
  <meta charset=\"utf-8\"/>
...
def generate_report(run_dir: Path) -> tuple[Path, Path]:
    ...
    meta_lines = [
        f"run_dir={run_dir}",
        f"frames_discovered={run_start.get('frames_discovered')}",
        f"config_hash={run_start.get('config_hash')}",
    ]

    css_path = artifacts_dir / "report.css"
    html_path = artifacts_dir / "report.html"
    _write_css(css_path)
    _write_html(html_path, title, meta_lines, sections)
