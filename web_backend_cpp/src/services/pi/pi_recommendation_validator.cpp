...
std::optional<std::string> semantic_update_reject_reason(const nlohmann::json& update,
    ...
    if (!meta.empty()) {
        ...
        if (meta.value("diagnostic_only", false) &&
            contains_any(reason, {"reconstruction quality", "reconstruction", "improves diagnostic accuracy", "improves", "degrades"})) {
    ...
    if (meta.contains("disabled_value") && !meta["disabled_value"].is_null() &&
        contains_any(reason, {"automatically disable", "auto-disabled", "0 means any", "0.0 means any", "means any"})) {
        if (const nlohmann::json* cur = get_dotted_ptr(base_config, path)) {
            if (*cur == meta["disabled_value"]) return "disabled_sentinel_misinterpreted";
        }
    }
