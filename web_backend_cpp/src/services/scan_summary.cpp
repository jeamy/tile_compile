...
nlohmann::json summarize_scan_job(const std::optional<Job>& job,
    ...
    if (!job.has_value()) {
        return {
            ...
            {"per_dir_results_total", 0},
            {"per_dir_results_truncated", false},
        };
    }
    ...
    return {
        ...
        {"errors", errors},
        {"errors_total", result.value("errors_total", static_cast<int>(errors.size()))},
        {"errors_truncated", result.value("errors_truncated", false)},
        {"warnings", warnings},
        {"warnings_total", result.value("warnings_total", static_cast<int>(warnings.size()))},
        {"warnings_truncated", result.value("warnings_truncated", false)},
        {"frames", frames},
        {"frames_total", result.value("frames_total", result.value("frames_detected", static_cast<int>(frames.size())))},
        {"frames_truncated", result.value("frames_truncated", false)},
        {"per_dir_results", per_dir_results},
        {"per_dir_results_total", result.value("per_dir_results_total", static_cast<int>(per_dir_results.size()))},
        {"per_dir_results_truncated", result.value("per_dir_results_truncated", false)},
    };
}
